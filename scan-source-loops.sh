...
# Source Code Cursor Loops Scanner
# This script scans only the actual source code for loops, excluding node_modules
...
echo -e "${BLUE}🔍 Scanning source code for cursor loops in SalesHub CRM...${NC}"
...
# Scan backend files (excluding bin directory)
echo -e "${YELLOW}📁 Scanning backend source files...${NC}"
backend_files=$(find backend -name "*.go" -type f -not -path "*/bin/*" 2>/dev/null || true)
...
# Scan frontend source files (excluding node_modules)
echo -e "${YELLOW}📁 Scanning frontend source files...${NC}"
frontend_files=$(find frontend/src -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" -type f 2>/dev/null || true)
...
echo "" >> "$TEMP_FILE"
echo "- **Total Source Files Scanned**: $total_files_scanned" >> "$TEMP_FILE"
...
echo -e "${GREEN}✅ Source code cursor loops scan completed!${NC}"
echo -e "${BLUE}📊 Summary:${NC}"
echo -e "  • Source files scanned: $total_files_scanned"
